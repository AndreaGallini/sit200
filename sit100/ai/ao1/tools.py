...
def write_csv_data(filename, data, the_header=None):
    with open(filename, 'w', newline='') as csvfile:
        if the_header or (isinstance(data, list) and isinstance(data[0], dict)):
            ...
            writer = csv.DictWriter(csvfile, fieldnames=the_header, delimiter=';')
            writer.writeheader()
        else:
...
def validate(input_data, data_schema):
    ...
    if len(input_data) != len(data_schema):
        print(f"expecting {len(data_schema)} parameters, got {len(input_data)}")
        return False
    if isinstance(data_schema, list):  # TODO: prune this branch after adapting models, accept only dict
        for i, v in enumerate(input_data.values()):
            ...
            if t == int:
                try:
                    ...
                    print(f"expecting integer values for {t} parameter, got {v}")
                    return False
            elif t == float:
                try:
                    v = float(v)
...
capacities = [17.5, 20, 30, 38, 53, 71, 94, 117, 148, 185, 222, 260, 300, 341, 407, 740, 925, 1110, 1295, 1480, 1665]


def cable_section_by_capacity(capacity):
    sections = [1.5, 2.5, 4, 6, 10, 16, 25, 35, 50, 70, 95, 120, 150, 185, 240, 280, 350, 420, 490, 560, 630]
